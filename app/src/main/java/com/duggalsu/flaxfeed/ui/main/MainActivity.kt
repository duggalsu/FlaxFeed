...
import com.duggalsu.flaxfeed.databinding.ActivityMainBinding
import com.duggalsu.flaxfeed.databinding.DialogEditFeedBinding
...
import com.duggalsu.flaxfeed.ui.about.AboutActivity
import com.duggalsu.flaxfeed.ui.discover.DiscoverActivity
import com.duggalsu.flaxfeed.ui.entries.EntriesFragment
import com.duggalsu.flaxfeed.ui.entrydetails.EntryDetailsActivity
import com.duggalsu.flaxfeed.ui.entrydetails.EntryDetailsFragment
import com.duggalsu.flaxfeed.ui.feeds.FeedAdapter
import com.duggalsu.flaxfeed.ui.feeds.FeedGroup
import com.duggalsu.flaxfeed.ui.feeds.FeedListEditActivity
import com.duggalsu.flaxfeed.ui.settings.SettingsActivity
...
class MainActivity : AppCompatActivity(), MainNavigator, AnkoLogger {
    ...
    companion object {
        ...
        private const val INTENT_UNREADS = "com.duggalsu.flaxfeed.intent.UNREADS"
        private const val INTENT_ALL = "com.duggalsu.flaxfeed.intent.ALL"
        private const val INTENT_FAVORITES = "com.duggalsu.flaxfeed.intent.FAVORITES"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        com.duggalsu.flaxfeed.App.db.feedDao().observeAllWithCount.observe(this@MainActivity, { nullableFeeds ->
                        ...
                        setOnMenuItemClickListener { item ->
                            when (item.itemId) {
                                R.id.mark_all_as_read -> doAsync {
                                    when {
                                        feedWithCount.feed.id == Feed.ALL_ENTRIES_ID -> com.duggalsu.flaxfeed.App.db.entryDao().markAllAsRead()
                                        feedWithCount.feed.isGroup -> com.duggalsu.flaxfeed.App.db.entryDao().markGroupAsRead(feedWithCount.feed.id)
                                        else -> com.duggalsu.flaxfeed.App.db.entryDao().markAsRead(feedWithCount.feed.id)
                                    }
                                }
                                R.id.edit_feed -> {
                                            ...
                                            .setPositiveButton(android.R.string.ok) { _, _ ->
                                                ...
                                                if (newName.isNotBlank() && (newLink.isNotBlank() || feedWithCount.feed.isGroup)) {
                                                    doAsync {
                                                        ...
                                                        com.duggalsu.flaxfeed.App.db.feedDao().update(newFeed)
                                                    }
                                                }
                                            }
                                ...
                                R.id.delete -> {
                                    AlertDialog.Builder(this@MainActivity)
                                            ...
                                            .setPositiveButton(android.R.string.ok) { _, _ ->
                                                doAsync { com.duggalsu.flaxfeed.App.db.feedDao().delete(feedWithCount.feed) }
                                            }.setNegativeButton(android.R.string.cancel, null)
                                            .show()
                                }
                                R.id.enable_full_text_retrieval -> doAsync { com.duggalsu.flaxfeed.App.db.feedDao().enableFullTextRetrieval(feedWithCount.feed.id) }
                                R.id.disable_full_text_retrieval -> doAsync { com.duggalsu.flaxfeed.App.db.feedDao().disableFullTextRetrieval(feedWithCount.feed.id) }
                            }
                            true
                        }
    ...
    private fun openInBrowser(entryId: String) {
        doAsync {
            com.duggalsu.flaxfeed.App.db.entryDao().findByIdWithFeed(entryId)?.entry?.link?.let { url ->
    ...
    private fun exportOpml() {
        val intent = Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
            type = "text/*"
            addCategory(Intent.CATEGORY_OPENABLE)
            putExtra(Intent.EXTRA_TITLE, "FlaxFeed_" + System.currentTimeMillis() + ".opml")
        }
    ...
    private fun parseOpml(opmlReader: Reader) {
        ...
        if (feedList.isNotEmpty()) {
            com.duggalsu.flaxfeed.App.db.feedDao().insert(*feedList.toTypedArray())
        }
    }

    private fun exportOpml(opmlWriter: Writer) {
        val feeds = com.duggalsu.flaxfeed.App.db.feedDao().all.groupBy { it.groupId }

...
package com.duggalsu.flaxfeed.service
...
import com.duggalsu.flaxfeed.App.Companion.context
import com.duggalsu.flaxfeed.data.entities.Entry
import com.duggalsu.flaxfeed.data.entities.Feed
import com.duggalsu.flaxfeed.data.entities.Task
import com.duggalsu.flaxfeed.data.entities.toDbFormat
...
class FetcherService : IntentService(FetcherService::class.java.simpleName) {
    ...
    companion object : AnkoLogger {
        ...
        const val ACTION_REFRESH_FEEDS = "com.duggalsu.flaxfeed.REFRESH"
        const val ACTION_MOBILIZE_FEEDS = "com.duggalsu.flaxfeed.MOBILIZE_FEEDS"
        const val ACTION_DOWNLOAD_IMAGES = "com.duggalsu.flaxfeed.DOWNLOAD_IMAGES"

        private const val THREAD_NUMBER = 3
        private const val MAX_TASK_ATTEMPT = 3

        private val IMAGE_FOLDER_FILE = File(com.duggalsu.flaxfeed.App.context.cacheDir, "images/")
        ...
        fun fetch(context: Context, isFromAutoRefresh: Boolean, action: String, feedId: Long = 0L) {
            ...
            when (action) {
                ...
                ACTION_DOWNLOAD_IMAGES -> downloadAllImages()
                    ...
                    var newCount = 0
                    if (feedId == 0L || com.duggalsu.flaxfeed.App.db.feedDao().findById(feedId)?.isGroup == true) {
                        newCount = refreshFeeds(feedId, acceptMinDate)
                    } else {
                        com.duggalsu.flaxfeed.App.db.feedDao().findById(feedId)?.let {
                            try {
        ...
        private fun showRefreshNotification(itemCount: Int = 0) {
            ...
            if (shouldDisplayNotification && itemCount > 0) {
                if (!MainActivity.isInForeground) {
                    val unread = com.duggalsu.flaxfeed.App.db.entryDao().countUnread
                    ...
                    if (unread > 0) {
                        ...
                        val notifBuilder = NotificationCompat.Builder(context, channelId)
                                ...
                                .setAutoCancel(true)
                                .setContentTitle(context.getString(R.string.flaxfeed_feeds))
                                .setContentText(text)
        ...
        fun addImagesToDownload(imgUrlsToDownload: Map<String, List<String>>) {
            if (imgUrlsToDownload.isNotEmpty()) {
                ...
                com.duggalsu.flaxfeed.App.db.taskDao().insert(*newTasks.toTypedArray())
            }
        }
        ...
        private fun mobilizeAllEntries() {
            ...
            val tasks = com.duggalsu.flaxfeed.App.db.taskDao().mobilizeTasks
            ...
            for (task in tasks) {
                ...
                com.duggalsu.flaxfeed.App.db.entryDao().findById(task.entryId)?.let { entry ->
                    entry.link?.let { link ->
                        try {
                                ...
                                response.body?.byteStream()?.let { input ->
                                    Readability4JExtended(link, Jsoup.parse(input, null, link)).parse().articleContent?.html()?.let {
                                        ...
                                        if (entryDescription == null || HtmlCompat.fromHtml(mobilizedHtml, HtmlCompat.FROM_HTML_MODE_LEGACY).length > HtmlCompat.fromHtml(entryDescription, HtmlCompat.FROM_HTML_MODE_LEGACY).length) { // If the retrieved text is smaller than the original one, then we certainly failed...
                                            ...
                                            com.duggalsu.flaxfeed.App.db.entryDao().update(entry)

                                            com.duggalsu.flaxfeed.App.db.taskDao().delete(task)
                                        }
                                    }
                                }
                            }
                ...
                if (!success) {
                    if (task.numberAttempt + 1 > MAX_TASK_ATTEMPT) {
                        ...
                        com.duggalsu.flaxfeed.App.db.taskDao().update(task)
                    }
                }
            }
        ...
        private fun downloadAllImages() {
            if (shouldDownloadPictures()) {
                val tasks = com.duggalsu.flaxfeed.App.db.taskDao().downloadTasks
                for (task in tasks) {
                    try {
                        ...
                        if (task.numberAttempt + 1 > MAX_TASK_ATTEMPT) {
                            ...
                            task.numberAttempt += 1
                            com.duggalsu.flaxfeed.App.db.taskDao().insert(task)
                        }
                    }
                }
            }
        ...
        private fun refreshFeeds(feedId: Long, acceptMinDate: Long): Int {
            ...
            val feeds: List<Feed>
            if (feedId == 0L) {
                feeds = com.duggalsu.flaxfeed.App.db.feedDao().allNonGroupFeeds
            } else {
                feeds = com.duggalsu.flaxfeed.App.db.feedDao().allFeedsInGroup(feedId)
            }
        ...
        private fun refreshFeed(feed: Feed, acceptMinDate: Long): Int {
            ...
            if (feed != previousFeedState) {
                com.duggalsu.flaxfeed.App.db.feedDao().update(feed)
            }

            // First we remove the entries that we already have in db (no update to save data)
            val existingIds = com.duggalsu.flaxfeed.App.db.entryDao().idsForFeed(feed.id)
            ...
            if (context.getPrefBoolean(PrefConstants.REMOVE_DUPLICATES, true)) {
                val existingTitles = com.duggalsu.flaxfeed.App.db.entryDao().findAlreadyExistingTitles(entries.mapNotNull { it.title })
            ...
            // Insert everything
            com.duggalsu.flaxfeed.App.db.entryDao().insert(*(entriesToInsert.toTypedArray()))
        ...
        private fun deleteOldEntries(keepDateBorderTime: Long, read: Long) {
            if (keepDateBorderTime > 0) {
                com.duggalsu.flaxfeed.App.db.entryDao().deleteOlderThan(keepDateBorderTime, read)
        ...
        private fun deleteEntriesImagesCache(keepDateBorderTime: Long) {
            if (IMAGE_FOLDER_FILE.exists()) {
                ...
                val favoriteIds = com.duggalsu.flaxfeed.App.db.entryDao().favoriteIds
